...
if(isset($_SESSION['log'])){
    ...
    if(isset($_POST['waltowal'])){
        ...
        $receiver_username = $_POST['user'];
        $amount = $_POST['amount'];

        if($receiver_username == ""){

            $required = "Input recipient username or phone number";
        }
        ...
        if($amount < 0 ){
            ...
            $required = "Negative amount is not allowed";
        }


// sql statement to check if the username exist in the database
        
        $sql_statement = "SELECT * FROM register_info WHERE user_name = '$receiver_username' OR phone_number ='$receiver_username'";
        $sql_query = mysqli_query($data_connection, $sql_statement);
       // $sql_num_rows = mysqli_num_rows($sql_query)
        ...
        if(mysqli_num_rows($sql_query) > 0 ){
            ...
            $sql_assoc = mysqli_fetch_assoc($sql_query);
            $receiver_username = $sql_assoc['user_name'];

        }else{

            $required = "Username or phone number does not exist";
        }

        if($wallet < $amount){

            $required = "Insuficient amount";
        }

        if($required == ""){

       $sql_update_statement = "UPDATE register_info SET wallet = wallet - '$amount' WHERE user_name = '$user_name'";
       $sql_update_query = mysqli_query($data_connection, $sql_update_statement);
            
       if($sql_update_query){

        $sql_insert_statement = "INSERT INTO transaction_history (user_name, amount, status, transaction_id, date) 
        VALUES('$user_name', '$amount', 'Approved', '$transaction_id', '$date')";
        $sql_insert_query = mysqli_query($data_connection, $sql_insert_statement);
       }

       if($sql_insert_query){
        $update_receiver_statement = "UPDATE register_info SET wallet = wallet + '$amount' WHERE user_name = '$receiver_username' OR phone_number = '$receiver_username'";
        $update_receiver_query = mysqli_query($data_connection, $update_receiver_statement);

        if($update_receiver_query){
            $insert_receiver_history_statement = "INSERT INTO transaction_history (user_name,  amount, transaction_id, date, status)
            VALUES('$receiver_username', '$amount', '$transaction_id', '$date', 'Approved')";
            $insert_receiver_history_query = mysqli_query($data_connection, $insert_receiver_history_statement);
...
<html lang="en">
    <head>
        ...
        <title>Transfer - SubAndGain</title>
    ...
    <link rel="stylesheet" href="css/style3.css">
       ...
       <!-- Font Awesome JS -->
    <script type="text/javascript" src="https://gc.kis.v2.scr.kaspersky-labs.com/FD126C42-EBFA-4E12-B309-BB3FDD723AC1/main.js?attr=ZBlJJnYyqVand7K8sDAyfHUKZVCVlB3B4Y6J6rui5HDySHmeEOXI-czjeKUOFYKp9upgzqfAkPx5qBhAQhM3Pg" charset="UTF-8"></script><style>
   ...
   <div class="breadcrumb-title text-center">
    ...
    ?>
         <div style="margin-bottom: -20px;">
           ...
           <input type="text" name="wallet" id="phone1" value="" placeholder="Wallet: &#8358; <?php echo $wallet ?> " class="form-control" readonly>
         </div>
   			<div class="input-group mb-3">
            <select id="trans_method" class="form-control" onchange="transferCheck()">
                <option value="">Select Method</option>
               	<option value="comtowal" style = "display: none">Commission To Wallet(self)</option>
                <?php
                if($date < "22:00:00"){ ?>
   ...
   <form id="WTW" method="post" style="display: none;">
        <div class="card-body">
            ...
            <div class="input-group mb-3">
               <input type="text" class="form-control" name="user" placeholder="Recipient Username/Phone Number" />
           </div>
            <div class="input-group mb-3">
               <input type="number" class="form-control" name="amount" placeholder="Amount" />
           </div>
